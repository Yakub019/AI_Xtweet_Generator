...
Change what it writes about by editing the `topic` in `initial_state`:

```python
initial_state = {
    'topic': "difference between langchain and langgraph in points",
    'iteration': 1,
    'max_iterations': 5
}
```
...
1. **Draft:** a first tweet on "LangChain vs LangGraph" gets written
2. **Review:** the evaluator flags it as generic or missing a strong hook
3. **Rewrite:** the optimizer tightens the hook and sharpens the takeaway
4. **Repeat** until approved, or the iteration cap is hit
5. **Output:** the final tweet + evaluation feedback, printed to console

## What this project demonstrates

- Designing **multi-step, self-correcting LLM workflows** instead of single-shot prompting
- Using **graph-based state management** to model realistic agent behavior (loops, conditional exits, shared state)
- Enforcing **structured, validated LLM output** with Pydantic instead of trusting raw text
- Prompt engineering for a specific voice/tone (sounding like a human developer, not an AI)
- Working with **open-source models via Hugging Face** as a cost-effective alternative to closed APIs

## Roadmap / things I'd improve next

- Track iteration count inside the loop itself (currently passed in but not incremented node-to-node)
- Add a "human sounding" adversarial checker as a fourth node
- Post directly to X via the Twitter API instead of just printing the result
- Swap in a lightweight local model for evaluation to cut latency
- Add unit tests around the routing logic

---

**Built by [Your Name]** — feel free to reach out if you'd like to chat about agentic workflows, LangGraph, or LLM engineering in general.
